...
export const createLoanApplication = AsyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (!amount || !purpose || !term) {
    return res.status(400).json(
      new ApiResponse(400, {}, 'All fields are required: amount, purpose, term', false)
    );
  }
  ...
  try {
    ...
    return res.status(201).json(
      new ApiResponse(201, loanApplication, 'Loan application created successfully', true)
    );
  } catch (error) {
    console.error('Error creating loan application:', error);
    return res.status(500).json(
      new ApiResponse(500, {}, 'Error creating loan application', false)
    );
  }
});

// Upload documents for loan application
export const uploadLoanDocuments = AsyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (!files || files.length === 0) {
    return res.status(400).json(
      new ApiResponse(400, {}, 'No files uploaded', false)
    );
  }
  ...
  try {
    ...
    if (!loanApplication) {
      return res.status(404).json(
        new ApiResponse(404, {}, 'Loan application not found or not authorized', false)
      );
    }
    ...
    return res.status(200).json(
      new ApiResponse(200, uploadedDocuments, 'Documents uploaded successfully', true)
    );
  } catch (error) {
    console.error('Error uploading documents:', error);
    return res.status(500).json(
      new ApiResponse(500, {}, 'Error uploading documents', false)
    );
  }
});

// Get loan application by ID
export const getLoanApplicationById = AsyncHandler(async (req: AuthRequest, res: Response) => {
  const { applicationId } = req.params;
  const userId = req.auth?.userId;
  ...
  try {
    ...
    return res.status(200).json(
      new ApiResponse(200, loanApplication, 'Loan application retrieved successfully', true)
    );
  } catch (error) {
    console.error('Error fetching loan application:', error);
    return res.status(500).json(
      new ApiResponse(500, {}, 'Error fetching loan application', false)
    );
  }
});

// Get all loan applications for a user
export const getUserLoanApplications = AsyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.auth?.userId;
  ...
  try {
    ...
    return res.status(500).json(
      new ApiResponse(500, {}, 'Error fetching loans', false)
    );
  }
});

// Update loan application status (admin only)
export const updateLoanApplicationStatus = AsyncHandler(async (req: AuthRequest, res: Response) => {
  const { applicationId } = req.params;
  ...
  if (!status || !['APPROVED', 'REJECTED', 'CANCELLED'].includes(status)) {
    return res.status(400).json(
      new ApiResponse(400, {}, 'Invalid status', false)
    );
  }
  ...
  try {
    ...
    if (!application) {
      return res.status(404).json(
        new ApiResponse(404, {}, 'Loan application not found', false)
      );
    }
    ...
    if (status === 'APPROVED') {
      if (!interestRate) {
        return res.status(400).json(
          new ApiResponse(400, {}, 'Interest rate is required for approval', false)
        );
      }
      ...
      if (!rejectionReason) {
        return res.status(400).json(
          new ApiResponse(400, {}, 'Rejection reason is required', false)
        );
      }
    ...
    if (status === 'APPROVED') {
      ...
      await prisma.loan.create({
        data: {
          loanNumber: `LOAN-${uuidv4().substring(0, 8).toUpperCase()}`,
    ...
    return res.status(200).json(
      new ApiResponse(200, updatedApplication, 'Loan application updated successfully', true)
    );
  } catch (error) {
    console.error('Error updating loan application:', error);
    return res.status(500).json(
      new ApiResponse(500, {}, 'Error updating loan application', false)
    );
  }
});

// Get all loan applications (admin only)
export const getAllLoanApplications = AsyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.auth?.userId;
  ...
  try {
    ...
    if (req.query.purpose) {
      filters.purpose = {
        contains: req.query.purpose as string,
    ...
    // Term filter
    if (req.query.term) {
      filters.term = parseInt(req.query.term as string);
    }
    ...
    // Sorting
    const sortField = req.query.sortBy as string || 'submittedAt';
    ...
    return res.status(500).json(
      new ApiResponse(500, {}, 'Error fetching loan applications', false)
    );
  }
});
...
export const getLoanById = AsyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (!loan) {
      return res.status(404).json(
        new ApiResponse(404, {}, 'Loan not found', false)
      );
    }
    ...
    return res.status(200).json(
      new ApiResponse(200, loan, 'Loan retrieved successfully', true)
    );
  } catch (error) {
    console.error('Error fetching loan:', error);
    return res.status(500).json(
      new ApiResponse(500, {}, 'Error fetching loan', false)
    );
  }
});

// Get all loans for a user
export const getUserLoans = AsyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.auth?.userId;
...
export const getAllLoans = AsyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.auth?.userId;
  ...
  try {
    ...
    const orderBy: any = {};
    orderBy["createdAt" as string] = sortOrder;

    // Get total count for pagination metadata
    const totalLoans = await prisma.loan.count({ where: whereConditions });
    ...
    return res.status(200).json(
      new ApiResponse(
        200, 
        { loans, pagination: paginationMeta }, 
        'Loans retrieved successfully', 
        true
      )
    );
...
export const makePayment = AsyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (loan.status !== 'ACTIVE') {
      return res.status(400).json(
        new ApiResponse(400, {}, `Cannot make payment on a ${loan.status.toLowerCase()} loan`, false)
      );
    }
    ...
    const payment = await prisma.payment.create({
      data: {
        ...
        reference: `PAY-${uuidv4().substring(0, 8).toUpperCase()}`,
    ...
    const updatedLoan = await prisma.loan.update({
      ...
      data: {
        ...
        status: loanStatus as LoanStatus
      }
    });
    ...
    return res.status(200).json(
      new ApiResponse(200, 
        { payment, transaction, loan: updatedLoan }, 
    ...
    return res.status(500).json(
      new ApiResponse(500, {}, 'Error processing payment', false)
    );
  }
});

// Update loan status (admin only)
export const updateLoanStatus = AsyncHandler(async (req: AuthRequest, res: Response) => {
  const { loanId } = req.params;
  ...
  try {
    ...
    return res.status(200).json(
      new ApiResponse(200, updatedLoan, 'Loan status updated successfully', true)
    );
  } catch (error) {
    console.error('Error updating loan status:', error);
    return res.status(500).json(
      new ApiResponse(500, {}, 'Error updating loan status', false)
    );
  }
});